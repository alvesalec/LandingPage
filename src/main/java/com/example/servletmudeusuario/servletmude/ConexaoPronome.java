...
//            Desconectando do DB
    ...
    //    Método para remover da tabela pronome tanto por id quanto por descrição
    public boolean inserir(String nome) {
        try {
            conectar();//Abrindo conexão com o banco
            pstmt = conn.prepareStatement("INSERT into tipo_pronome (cdescricao) VALUES (lower(?))");
//                          Setando o valor dos parâmetro
    ...
    public boolean remover(String nomeCampo, String parametro) {
        ...
        try {
            ...
            if (nomeCampo.equals("cdescricao")) {
                pstmt = conn.prepareStatement("DELETE from tipo_pronome WHERE " + nomeCampo + " = ?");
                pstmt.setString(1, parametro);
            }else if (nomeCampo.equals("ncdpronome")){
                pstmt = conn.prepareStatement("DELETE from  tipo_pronome WHERE " + nomeCampo + " = ?");
    ...
    //    Método para alterar a tabela pronome que recebe como parametro o campo de condição,
//    a condição e o novo valor
    public int alterar( String campoParametro, String valorComparativo, String novoValor) {
        try {
            ...
            pstmt = conn.prepareStatement("UPDATE tipo_pronome SET cdescricao = lower(?) WHERE "+ campoParametro+ " = ?");

            if (campoParametro.equals("ncdpronome") ){
    ...
    //    Método para buscar tudo da tabela pronome
    public ResultSet buscarTudo(){
        ...
        try{
            ...
            pstmt = conn.prepareStatement("SELECT * FROM tipo_pronome " );
    ...
    //    Método para buscar em específico da tabela pronome, pode procurar tanto por id quanto por
//     descrição
    public ResultSet buscar(String campoParametro, String valorComparacao){
        ...
        try{
            ...
            if (campoParametro.equals("ncdpronome") ){
                //Comando SQL Para Selecionar os dados das tabelas
                pstmt = conn.prepareStatement("SELECT * FROM tipo_pronome WHERE " + campoParametro + " = " + Integer.valueOf(valorComparacao));
            }else {
                //Comando SQL Para Selecionar os dados das tabelas
                pstmt = conn.prepareStatement("SELECT * FROM tipo_pronome WHERE " + campoParametro + " = '" + valorComparacao+"'");
            }
