...
//            Desconectando do DB
    ...
    //    Método para remover da tabela genero tanto por id quanto por descrição
    public boolean inserir(String nome) {
        try {
            conectar();//Abrindo conexão com o banco
            pstmt = conn.prepareStatement("INSERT into tipo_genero (cdescricao) VALUES (lower(?))");
//                            Setando o valor dos parâmetro
    ...
    public boolean remover(String nomeCampo, String parametro) {
        try {
            ...
            if (nomeCampo.equals("cdescricao")) {
                pstmt = conn.prepareStatement("DELETE from tipo_genero WHERE " + nomeCampo + " = ?");
                pstmt.setString(1, parametro);
            }else if(nomeCampo.equals("ncdgenero")) {
                pstmt = conn.prepareStatement("DELETE from  tipo_genero WHERE " + nomeCampo + " = ?");
    ...
    //    Método para alterar a tabela genero que recebe como parametro o campo de condição,
//    a condição e o novo valor
    public int alterar( String campoParametro, String valorComparativo, String novoValor) {
        try {
            ...
            pstmt = conn.prepareStatement("UPDATE tipo_genero SET cdescricao = lower(?) WHERE "+ campoParametro+ " = ?");

            if (campoParametro.equals("ncdgenero") ){
    ...
    //    Método para buscar tudo da tabela genero
    public ResultSet buscarTudo(){
        ...
        try{
            ...
            pstmt = conn.prepareStatement("SELECT * FROM tipo_genero " );
    ...
    //    Método para buscar em específico da tabela genero, pode procurar tanto por id quanto por
//     descrição
    public ResultSet buscar(String campoParametro, String valorComparacao){
        ...
        try{
            ...
            if (campoParametro.equals("ncdgenero") ){
                //Comando SQL Para Selecionar os dados das tabelas
                pstmt = conn.prepareStatement("SELECT * FROM tipo_genero WHERE " + campoParametro + " = " + Integer.valueOf(valorComparacao));
            }else {
                //Comando SQL Para Selecionar os dados das tabelas
                pstmt = conn.prepareStatement("SELECT * FROM tipo_genero WHERE " + campoParametro + " = '" + valorComparacao+"'");
            }
